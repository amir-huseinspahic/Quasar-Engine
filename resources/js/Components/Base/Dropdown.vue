...
<template>
    <div class="relative inline-block text-left">
        <div>
            <button type="button" class="bg-gray-50 inline-flex w-full justify-center gap-x-1.5 rounded-md px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50" id="menu-button" aria-expanded="true" aria-haspopup="true" @click="toggleDropdown">
                {{ $page.props.auth.user.name }}
                <ChevronDownIcon class="-mr-1 size-5 text-gray-400 transform transition-all ease-out" :class="isDropdownExpanded ? 'rotate-180' : 'rotate-0'" />
            </button>
        </div>
